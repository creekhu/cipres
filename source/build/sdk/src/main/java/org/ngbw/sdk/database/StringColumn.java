...
class StringColumn extends Column<String> {
	...
	private class StringValue extends MemoryValue<String> {
		...
		public void setParamValue(PreparedStatement statement, int index) throws SQLException
		{
			...
			if (m_memValue.length() > m_maxLength)
			{
				if (m_truncate)
				{
					m_memValue = m_memValue.substring(0, Math.min(m_memValue.length(), m_maxLength));
				} else
				{
					throw new ValidationException(getName() + " exceeds maximum length of " + String.valueOf(m_maxLength));
				}
			}
	...
	{
		this(name, nullable, maxLength, false);
	}
	 ...
	 * @param truncate
	 */
	StringColumn(String name, boolean nullable, int maxLength, boolean truncate)
	{
		super(name, nullable);
