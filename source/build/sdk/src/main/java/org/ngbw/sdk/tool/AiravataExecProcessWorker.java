/*
 * AiravataExecProcessWorker.java
 */

/*
 *      To use Airavata for job submission, some prerequisites must be met.
 *      - Scigap admin must register the gateway
 *      - Scigap admin must provide authentication credential tarball
 *      - Gateway admin must request ssh key and add to remote host 
 *        authorized_keys
 *      - remote resource must be registered
 *      - fork application must be registered, deployed, etc.
 *      - Airavata libraries must be downloaded with something like:
 *      git clone https://github.com/SciGaP/PythonClients.git
 *      (append_tuple in airavata-client.properties needs to point to
...
import org.ngbw.sdk.common.util.AiravataExecProcessRunner;
...
/**
 * AiravataExecProcessWorker
 * 
 *
 * @author Terri Liebowitz Schwartz 
 *
 */
public class AiravataExecProcessWorker extends BaseProcessWorker 
{
	...
	private static final Log m_log = LogFactory.getLog(AiravataExecProcessWorker.class.getName());
	...
	private Class<AiravataExecProcessRunner> m_runnerClass;

	/** 
		Constructors
	*/
	public AiravataExecProcessWorker(RunningTask rt) 
	...
	public AiravataExecProcessWorker(String jobhandle) 
		throws PropertyException, IOException, InstantiationException, 
		IllegalAccessException, ClassNotFoundException, Exception
	{
		super(jobhandle);
	...
	*/
	...
	{
		AiravataExecProcessRunner runner = null;
	...
	protected AiravataExecProcessRunner getProcessRunner() throws Exception
	{
		AiravataExecProcessRunner runner;
		try
		{
	...
	*/
	...
	{
		String jobID;
		AiravataExecProcessRunner runner = getProcessRunner(); 
		...
		*/
		...
		m_log.debug("Running ssh command: " + " " + m_login + " " + finalCommand);
	...
	public void cancelJob() throws Exception
	{
		...
		m_log.debug("m_rt.getRemoteJobId(): " + m_rt.getRemoteJobId());
	...
	private void setMyMembers(Properties p) throws PropertyException
	{
		...
		try
		{
			m_runnerClass = (Class<AiravataExecProcessRunner>)Class.forName(className);
		}
		catch(Exception e)
		{
			m_log.debug("", e);
