...
public abstract class BaseProcessWorker 
{
	...
	static
	{
		...
		{
			log.warn("Invalid max.file.size or max.dir.size property.");
		}
	}
	...
	********************************************************************************************/
	...
	/*
		Todo:
		The implementation of readFilediffers from FileHandler.getStream() only in that it does a
		number of retries.  That implementation should be in a FileHandler base class instead
		of here and in the specific process workers.  The retry details should be obtained
		from the registry for a specific resource and passed to the filehandler.
	*/
	...
	public void getResults(String localPath) throws Exception
	{
		...
		if (msg == null)
		{
			if (localPath == null && resultsTooLarge(fh))
			{
				...
				" by a script that runs at 6 am, 12 am, 6 pm, and 12 pm Pacific Time. You can contact " +
	...
	/*
		This doesn't throw exception.  We want loadResults to complete
		even if something goes wrong here;  too much has been going wrong.
		Can check statistics_event table to find out how many and which jobs
		are failing to store charege and how many are doing ok.  Then look at logs for more
		detail about the failures.
	*/
	protected void storeJobCharge() throws Exception
	{
		...
		{
			// we already did this on a previous attempt.
			return;
		}
		Long sus = null;
		try
		{
			...
			sus = jc.getCharge();
			log.info("JC - calculated jobcharge of " + sus);
			StatisticsEvent se = new StatisticsEvent(m_jobHandle);
			se.setEventName("CALCULATE_SUS_SUCCESS");
			se.setEventValue("" + sus);
			se.save();
		...
		{
			log.error("JC - error calculating jobcharge", e);
			StatisticsEvent se = new StatisticsEvent(m_jobHandle);
	...
	*/
	...
	{
		log.debug("MAX_FILE_SIZE=" + MAX_FILE_SIZE + ", MAX_DIR_SIZE=" + MAX_DIR_SIZE);
		if (MAX_FILE_SIZE == 0 && MAX_DIR_SIZE == 0)
		{
			return false;
		}
		long[] sizes = resultsSize(fh, m_workingDir);
		log.debug("ResultsTooLarge found total dir size = " + sizes[0] + ", largest file size = " + sizes[1]);
	...
	********************************************************************************************/
	private void initialize() throws Exception
	{
		...
		if (m_rt.getSprops() != null)
		{
			...
			m_maxRunHours = SchedulerProperties.getRunHours(m_schedulerConf);
		}
		// This is optional.
		String param;
