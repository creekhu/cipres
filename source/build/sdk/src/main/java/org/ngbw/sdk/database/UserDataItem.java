...
public class UserDataItem extends VersionedRow implements SourceDocument, FolderItem, Comparable<UserDataItem> {
	...
	private final Column<String> m_label = new StringColumn("LABEL", false, 1023);
	private final Column<Long> m_enclosingFolderId = new LongColumn("ENCLOSING_FOLDER_ID", false);
	...
	private UserDataItem()
	{
		...
		setUUID(UUID.randomUUID().toString().toUpperCase().replaceAll("-", ""));
		construct(m_comment, m_dataFormat, m_dataType, m_entityType, m_validated, m_UUID, m_sourceDocumentId, m_groupId, m_userId, m_creationDate, m_label, m_enclosingFolderId);
	}
	...
	public static UserDataItem findDataItem(String path) throws IOException, SQLException
	{
		...
		int offset = path.lastIndexOf(Folder.SEPARATOR);
