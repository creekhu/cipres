...
limit = 365 
gracePeriod = 30 
emailSubject = "URGENT Usage Notice from Portal"
emailContentsFile= os.path.expandvars("${SDK_VERSIONS}/deleteWarning1.txt")
fromaddr = "admin@web.portal"
ccaddr = "admin@web.portal"
...
def hasData(userId):
    ...
    # This is the same query that our java code for User.deleteFolders() uses to find
    # and then delete all the user's folders.

    query = "select folder_id from folders where user_id = %d" % userId

    # print "hasData query is '%s'" % query
    cur = conn.cursor()
    cur.execute(query)
    ...
    for row in cur:
        numberOfFolders += 1
    # print "This user has %d folders" % numberOfFolders
