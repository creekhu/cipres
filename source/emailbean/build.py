...
def main(argv):
    try:
        ...
        module_build_dir = path.join(top_dir, "build", "emailbean")

        if prepare_target:
            print "\npreparing the email bean source code..."

            module_build_src_dir = path.join(module_build_dir, "src")
            conf_module_src_dir = path.join(conf_dir, "emailbean", "src")
