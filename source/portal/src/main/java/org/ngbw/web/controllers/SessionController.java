...
/**
 * Controller class to handle NGBW web application user session management.
 * Parent class of all NGBW web controller classes.
 *
...
public class SessionController
{
	 ...
	 *================================================================*/
	/**
	 * Default constructor.  A SessionController object should only be
	 * instantiated with this constructor if it represents a workbench
	 * session in which the user is not yet authenticated.
	 */
	...
	/**
	 * Constructor taking an authenticated WorkbenchSession argument.  The
	 * argument must be a valid WorkbenchSession object in order to establish
	 * the proper connection with the model layer that is required to interact
	 * with the workbench.
	 *
	 ...
	 * 									object is null.
	 */
	public SessionController(WorkbenchSession workbenchSession)
	throws IllegalArgumentException {
		if (workbenchSession == null)
			throw new IllegalArgumentException("The provided workbench session is null.");
		else this.workbenchSession = workbenchSession;
	}
	 ...
	 *
	 * @return	The WorkbenchSession object representing the
	 * 			current authenticated user's workbench session.
	 */
	 ...
	 *
	 * @return	The Workbench object associated with the current user session.
	 ...
	 *
	 * @return	true if a user is currently logged in to the application,
	 * 			false otherwise.
	 */
	 ...
	 *
	 * @return	The User object representing the current authenticated user.
	 * 			Returns null if no authenticated workbench session is present,
	 ...
	 *
	 * @return	The current authenticated user's encrypted password.
	 * 			Returns null if no authenticated workbench session is present,
	 * 			or if an error occurs.
	 */
	...
	/**
	 * Retrieves the current authenticated user's role.  This is used to determine
	 * the user's web application permissions and functionality.
	 *
	 * @return	The UserRole object representing the current
	 * 			authenticated user's role in the application.
	 * 			Returns null if no authenticated workbench session is present,
	 * 			or if an error occurs.
	 */
	 ...
	 *
	 * @return	The Set of all NGBW user Groups that the
	 * 			current authenticated user is a member of.
	 * 			Returns null if no authenticated workbench session is present,
	 * 			or if an error occurs.
	 */
	...
	/**
	 * Determines whether the current user is registered.  Registered users
	 * have access to a greater variety of features and functionality than
	 * guest users.
	 *
	 ...
	 *
	 * @return	The value of the specified user preference.
	 * 			Returns null if no authenticated user is logged in,
	...
	/**
	 * Saves the specified preference value to the currently authenticated user's
	 * stored map of user preferences.
	 *
	 * @param preference	The key or label of the preference to be saved.
	 * @param value			The value of the preference to be saved.
	 *
	 * @return	true if the user preference was successfully saved,
	 * 			false otherwise
	 */
	...
	/**
	 * Removes the specified preference value from the currently authenticated user's
	 * stored map of user preferences.
	 *
	 * @param preference	The key or label of the preference to be cleared.
	 * @param value			The value of the preference to be cleared.
	 *
	 * @return	true if the user preference was successfully cleared,
	 * 			false otherwise
	 */
	 ...
	 *
	 * @return	The user with the specified username, if present.
	 * 			Returns null if no user exists with this username,
	 * 			if the argument username is null,
	 * 			or an error occurs.
	 */
	 ...
	 *
	 * @return	The user with the specified email address, if present.
	 * 			Returns null if no user exists with this email address,
	 * 			if the argument email address is null,
	 * 			or an error occurs.
	 */
	...
	/**
	 * Determine whether the current has missing required profile information.
	 * The only thing currently required is that STANDARD users must have
	 * institution and country information.  Other types of users will be
	 * ignored; including GUEST users who are NOT allowed to update their
	 * profile information.
	 * 
	 * @author - Mona Wong
	 ...
	 *================================================================*/
	/**
	 * Creates and registers a fully populated workbench User object for the
	 * user with the specified information.  It is assumed that the argument
	 * values have already been properly validated according to the client's
	 ...
	 * @param institution	The primary institution with which the user
	 * 						attempting to register is affiliated.
	 * @param role			The UserRole representing this user's intended role.
	 *
	 * @return	The ValidationResult object representing the outcome of the
	 * 			user account registration. Returns null if an error occurs.
	 */
	...
	{
		return registerUser(username, password, email, firstName, lastName, institution, country, UserRole.STANDARD);
	}
	...
	private ValidationResult doRegisterUser(String username, String password, String email,
		...
		if (username == null)
		{
			logger.debug("User could not be created because the provided username is null.");
			return null;
		} else if (password == null)
		{
			logger.debug("User could not be created because the provided password is null.");
			return null;
		}
		try
		{
			...
			User user = workbench.getNewUserInstance();
			...
			if (result.isValid()) {
				info("User account \"" + username + "\" (role \"" + role.toString() +
	...
	/**
	 * Creates and registers a fully populated workbench User object with
	 * a randomized username, representing a guest user account.
	 *
	 * @return	The ValidationResult object representing the outcome of the
	 * 			guest account registration. Returns null if an error occurs.
	 */
	public ValidationResult registerGuestUser()
	{
		...
		WorkbenchSession session = getWorkbenchSession();
		if (result != null && result.isValid() && session != null) try
		{
			...
			info("Default folder successfully created for guest user \"" +
				guestName + "\".");
		} catch (Throwable error)
		{
			reportError(error, "Error creating default folder for guest user \"" +
				guestName + "\"");
		}
		return result;
	}
	 ...
	 *
	 * @return	The WorkbenchSession object representing the newly activated
	 * 			user session, if the user successfully passes authentication.
	 * 			Returns null if an error occurs.
	 *
	 * @throws	UserAuthenticationException if the user supplies an invalid
	 * 			username and/or password.
	 */
	public WorkbenchSession login(String username) throws UserAuthenticationException
	{
		return doLogin(username, null);
	}
	...
	private WorkbenchSession doLogin(String username, String password) throws UserAuthenticationException
	{
		if (username == null) {
			logger.debug("User could not be logged in because the provided username is null.");
			return null;
		} else try {
			Workbench workbench = getWorkbench();
			if (workbench.hasActiveSession(username)) {
				//TODO: do the right thing
				info("User \"" + username + "\" is attempting to log in, but " +
			...
			if (session == null) {
				logger.debug("The workbench session retrieved when attempting to authenticate "+
	...
	/**
	 * Retrieves the active WorkbenchSession owned by the specified user,
	 * if the user is currently logged into the NGBW web application.
	 *
	 * @param username			The username of the user whose active session
	 * 							is to be retrieved.
	 * @param encryptedPassword	The (encrypted) password of the user whose
	 * 							active session is to be retrieved.
	 * @return	The WorkbenchSession object representing the currently active
	 * 			user session, if the user successfully passes authentication.
	 * 			Returns null if an error occurs.
	 *
	 * @throws	UserAuthenticationException	if the user supplies an invalid
	 * 			username and/or (encrypted) password.
	 */
	public WorkbenchSession getActiveSession(String username, String encryptedPassword)
	...
	/**
	 * Logs the current authenticated user out of the NGBW web application.
	 * Any remaining WorkbenchSession reference must still be cleaned up, however.
	 *
	 ...
	 *
	 * @return	true if the user's password is successfully changed,
	 * 			false otherwise.
	 */
	...
	/**
	 * Updates an existing user's password with the specified new password.
	 * It is assumed that the argument values have already been properly
	 * validated according to the client's particular business rules.
	 *
	 * @param oldPassword	The user's current password.
	 * @param newPassword	The user's new password.
	 *
	 * @return	true if the user's password was successfully updated,
	 * 			false otherwise
	 *
	 * @throws UserAuthenticationException if the user supplies an invalid
	 * 			current password.
	...
	/**
	 * Updates an existing user with the specified information.  It is assumed
	 * that the argument values have already been properly validated according
	 * to the client's particular business rules.
	 *
	...
	public ValidationResult editUser(String email, String firstName, String lastName,
		...
		try
		{
			...
			if (institution != null && institution.trim().equals("") == false)
				user.setInstitution(institution);
			else user.setInstitution(null);
			
			if ( country != null && country.trim().equals ( "" ) == false )
				user.setCountry ( country );
			else
			...
			/* Commented out following code because we now allow admin to change their own account info also.
			if (user.getRole() != UserRole.STANDARD)
			{
				result.addError("Only 'standard' Cipres web site accounts can be modified here. Your account has role " + user.getRole().toString());
				return result;
			}
			*/

			// Make sure email isn't changed to email address already in use by another STANDARD user.
			User userFromEmail = User.findUserByEmailAndRole(user.getEmail(), UserRole.STANDARD.toString());
