...
<#if (.node.@issimple[0]?exists &&  .node.@issimple[0]?contains("1") ) >
	...
	<#switch .node.@type>
		...
		<#case "InFile">
			...
			<s:select name="${name}" headerKey='' headerValue='' list="%{
		
			getDataForParameter('${name}')}" onchange="resolveParameters()"/>	
			</@compress>
		...
		<#case "Excl">
			...
			<#if size gt 3> <#-- make it a drop down list -->
...
<#macro vlist>	
	...
	<s:select name="${name}" 
	    ...
	    <#assign size = .node.value?size> 
			<#list .node.value as value>'${.node.value[value_index]}':'${.node.label[value_index]}'
				<#if .node.value?seq_index_of(value) lt (size-1)>, </#if>
			</#list>}" onchange="resolveParameters()"/>
    </@compress>
