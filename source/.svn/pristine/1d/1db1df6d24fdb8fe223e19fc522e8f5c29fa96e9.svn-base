...
public class UsageLimitSetter
{
	...
	public UsageLimitSetter(Application app)
	{
		this.app = app;
	}

	public UsageLimitSetter(User user)
	{
		this.user = user;
	}
	...
	public void setXsedeSU(Long limit) throws Exception
	{
		if (user != null)
		{
			user.preferences().put(UsageLimit.XSEDE_SU_LIMIT, limit.toString());
			user.save();
		} else
		{
			app.preferences().put(UsageLimit.XSEDE_SU_LIMIT, limit.toString());
			app.save();
		}
	}
	...
	public void setOtherSU(Long limit) throws Exception
	{
		if (user != null)
		{
			user.preferences().put(UsageLimit.OTHER_SU_LIMIT, limit.toString());
			user.save();
		} else
		{
			app.preferences().put(UsageLimit.OTHER_SU_LIMIT, limit.toString());
			app.save();
		}
	}
	...
	/* 
		Main program.  Print all output (including errors) to stdout so that we
		can run with stderr redirected to /dev/null to avoid seeing all the sdk logging. 
	*/
	static int index;
		...
		while (index < args.length && args[index].startsWith("-") && args[index].length() == 2)
		{
			switch(args[index++].charAt(1))
			{
				...
				case 'u':
					...
					User user = User.findUserByUsername(sval);
					if (user == null)
					{
