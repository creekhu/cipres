	...
	<#switch .node.@type>
		...
		${name} 
		    ...
		    <#if .node.attributes.precond?is_node>
				  	 ...
				  	 Precond= ${.node.attributes.precond[0].code}  	 
    			...
    			<#assign precondString=precondString?replace(" =~ /", ".match(\"")> 
		     	...
		     	<#assign precondString=precondString?replace("( !~ /\\^)(\\W*)(\\w*)(\\W*)", ".match(\"^"+"$2$3$4"+"==false","r")> 
	
				<#assign precondString = precondString?replace("\\$(\\w*)","$1"+"_","r")>
			...
			<#if .node.attributes.ctrls?exists>	 		  	
			 <#assign size = .node.attributes.ctrls.ctrl?size> 			
				<#list .node.attributes.ctrls.ctrl as ctrl>				
					...
					<#assign controlString=controlString?replace("( !~ /\\^)(\\W*)(\\w*)(\\W*)", ".match(\"^"+"$2$3$4"+"==false","r")> 
					
					<#assign controlString = controlString?replace("\\$(\\w*)","$1"+"_","r")>
