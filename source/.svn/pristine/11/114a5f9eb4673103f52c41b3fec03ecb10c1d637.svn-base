...
public class Store implements Datastore
{
	...
	protected static String extensions = "creation,termination,get";
	...
	*/
	...
	/*
		...
		Side effects: may remove .info if .bin doesn't exist.  May remove
		both if .info can't be parsed.

		Can throw IOException if filesystem errors occur.
	*/
	@Override
	public FileInfo getFileInfo(String id) throws Exception
	{
		...
		// If Info exists but bin doesn't, probably because we crashed in terminate call.
		// Delete the info 
		...
		try
		{
			fileInfo = mapper.readValue(ifile, FileInfo.class);
		}
		catch (JsonProcessingException jpe)
		{
			log.warn("Can't parse " + ifile.getAbsolutePath());
			// These will be deleted by cron or whatever when past a certain age.  Could
			// delete them right now but am keeping for debugging, at least for now.
			//bfile.delete();
			//ifile.delete();
			return null;
		}
		fileInfo.offset =  bfile.length();
		fileInfo.lastModified = bfile.lastModified();
		return fileInfo;
	}
	...
	{ 
		...
		text = "username=" + fi.username + "\n";
		text += "pathname=" + getBinPath(id) + "\n";
		text += "suggestedFilename=" + fi.suggestedFilename + "\n";
		text += "id=" + id + "\n";
		return text;
	} 
