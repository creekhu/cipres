...
public class FolderManager extends SessionManager
{
		...
		if (validateFolder()) {
			...
			if (folder == null) try {
				...
				if (folder != null) {
					...
					setCurrentFolder(folder);
					refreshFolders();
				} else reportUserError("Folder \"" + getLabel() + "\" could not be created.");
			...
			// otherwise, we are trying to edit an existing folder
				...
				if (folder != null) {
					...
					setCurrentFolder(folder);
					refreshFolders();
				} else reportUserError("Folder \"" + oldLabel + "\" could not be edited.");
			} catch (UserAuthenticationException error) {
				reportUserError(error.getMessage());
			}
			clearWorkingFolder();
	...
	/*================================================================
	 * Folder display page property accessor methods
	 *================================================================*/
	...
	protected void setParentFolder(Folder folder) {
		if (folder == null)
			clearParentFolder();
		else setSessionAttribute(PARENT_FOLDER, folder);
	}
