...
public class CiCommunicator 
{
	...
	public void deleteUrl(String url) throws CiCipresException
	{
		...
		{
			throw new CiCipresException(getErrorData(response), response.getStatus());
		} 
	}
	...
	public String getRawTextFromUrl(String url) throws CiCipresException
	{
		log.debug("getRawTextFromUrl " + url);
		WebTarget target = client.target(url);

		// Do this, or omit Accept header?
		Response response = target.request("text/plain", "text/html", "application/xml", "application/json").get();

		log.debug("" + response.getStatus());
		if (response.getStatus() != 200)
		{
			throw new CiCipresException(getRawError(response), response.getStatus());
		} 
	...
	*/
	...
	private static ErrorData getRawError(Response response)
	{
		ErrorData ed = null;
		...
		return new ErrorData(text, text, ErrorData.GENERIC_COMM_ERROR);
	}
	...
	private Object readResponse(Response response) throws CiCipresException
	{
		...
		{
			throw new CiCipresException(e, response.getStatus());
		}
	}
