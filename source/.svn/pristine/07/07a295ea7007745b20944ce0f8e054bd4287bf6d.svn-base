...
public class ListPage<T> implements Page<T>
{
	...
	public ListPage(List<T> elements) {
		this(elements, 20);
	}
	
	public ListPage(List<T> elements, int pageSize) {
	...
	public void sortElements(Comparator<T> comparator) {
		if (elements == null || comparator == null) return;
		else {
			T[] elementsArray = (T[])elements.toArray();
			Arrays.sort(elementsArray, comparator);
			setElements(Arrays.asList(elementsArray));
			setPageNumber(0);
		}
	}
	...
	public List<T> getThisPageElements() {
		if (getTotalNumberOfElements() < 1) return null;
		else return getElements().subList(getThisPageFirstElementNumber(),
